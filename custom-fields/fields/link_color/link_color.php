 ...
 *
 */
if ( ! class_exists( 'FPBLOCK_Field_link_color' ) ) {
  class FPBLOCK_Field_link_color extends FPBLOCK_Fields {
    ...
    public function render() {
      ...
      $color_props = array(
        'color'    => esc_html__( 'Normal', 'fpblock' ),
        'hover'    => esc_html__( 'Hover', 'fpblock' ),
        'active'   => esc_html__( 'Active', 'fpblock' ),
        'visited'  => esc_html__( 'Visited', 'fpblock' ),
        'focus'    => esc_html__( 'Focus', 'fpblock' )
      );
      ...
      foreach ( $color_props as $color_prop_key => $color_prop_value ) {
        ...
        if ( ! empty( $args[$color_prop_key] ) ) {
          ...
          echo '<div class="fpblock--left fpblock-field-color">';
          echo '<div class="fpblock--title">'. esc_attr( $color_prop_value ) .'</div>';
          echo '<input type="text" name="'. esc_attr( $this->field_name( '['. $color_prop_key .']' ) ) .'" value="'. esc_attr( $value[$color_prop_key] ) .'" class="fpblock-color"'. $default_attr . $this->field_attributes() .'/>';
          echo '</div>';
