...
if ( ! class_exists( 'FPBLOCK_Setup' ) ) {
  class FPBLOCK_Setup {
    ...
    public function __construct() {
      ...
      // Init action
      do_action( 'fpblock_init' );

      // Setup textdomain
      self::textdomain();

      add_action( 'after_setup_theme', array( 'FPBLOCK', 'setup' ) );
      add_action( 'init', array( 'FPBLOCK', 'setup' ) );
      add_action( 'switch_theme', array( 'FPBLOCK', 'setup' ) );
      add_action( 'admin_enqueue_scripts', array( 'FPBLOCK', 'add_admin_enqueue_scripts' ) );
      add_action( 'wp_enqueue_scripts', array( 'FPBLOCK', 'add_typography_enqueue_styles' ), 80 );
      add_action( 'wp_head', array( 'FPBLOCK', 'add_custom_css' ), 80 );
      add_filter( 'admin_body_class', array( 'FPBLOCK', 'add_admin_body_class' ) );
    ...
    public static function setup() {
      ...
      $params = array();
      if ( class_exists( 'FPBLOCK_Options' ) && ! empty( self::$args['admin_options'] ) ) {
        foreach ( self::$args['admin_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            FPBLOCK_Options::instance( $key, $params );
      ...
      $params = array();
      if ( class_exists( 'FPBLOCK_Customize_Options' ) && ! empty( self::$args['customize_options'] ) ) {
        foreach ( self::$args['customize_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            FPBLOCK_Customize_Options::instance( $key, $params );
      ...
      $params = array();
      if ( class_exists( 'FPBLOCK_Metabox' ) && ! empty( self::$args['metabox_options'] ) ) {
        foreach ( self::$args['metabox_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            FPBLOCK_Metabox::instance( $key, $params );
      ...
      $params = array();
      if ( class_exists( 'FPBLOCK_Nav_Menu_Options' ) && ! empty( self::$args['nav_menu_options'] ) ) {
        foreach ( self::$args['nav_menu_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            FPBLOCK_Nav_Menu_Options::instance( $key, $params );
      ...
      $params = array();
      if ( class_exists( 'FPBLOCK_Profile_Options' ) && ! empty( self::$args['profile_options'] ) ) {
        foreach ( self::$args['profile_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            FPBLOCK_Profile_Options::instance( $key, $params );
      ...
      $params = array();
      if ( class_exists( 'FPBLOCK_Taxonomy_Options' ) && ! empty( self::$args['taxonomy_options'] ) ) {
        ...
        foreach ( self::$args['taxonomy_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            FPBLOCK_Taxonomy_Options::instance( $key, $params );
      ...
      if ( class_exists( 'FPBLOCK_Widget' ) && class_exists( 'WP_Widget_Factory' ) && ! empty( self::$args['widget_options'] ) ) {
        ...
        foreach ( self::$args['widget_options'] as $key => $value ) {
          if ( ! isset( self::$inited[$key] ) ) {
            ...
            self::$inited[$key] = true;
            $wp_widget_factory->register( FPBLOCK_Widget::instance( $key, $value ) );
      ...
      $params = array();
      if ( class_exists( 'FPBLOCK_Comment_Metabox' ) && ! empty( self::$args['comment_options'] ) ) {
        foreach ( self::$args['comment_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            FPBLOCK_Comment_Metabox::instance( $key, $params );
      ...
      $params = array();
      if ( class_exists( 'FPBLOCK_Shortcoder' ) && ! empty( self::$args['shortcode_options'] ) ) {
        foreach ( self::$args['shortcode_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            FPBLOCK_Shortcoder::instance( $key, $params );

          }
        }

        // Once editor setup for gutenberg and media buttons
        if ( class_exists( 'FPBLOCK_Shortcoder' ) && ! empty( self::$shortcode_instances ) ) {
          foreach ( self::$shortcode_instances as $instance ) {
            if ( ! empty( $instance['show_in_editor'] ) ) {
              FPBLOCK_Shortcoder::once_editor_setup();
              break;
            }
          }
        }
    ...
    public static function include_plugin_file( $file, $load = true ) {
      ...
      $path     = '';
      $file     = ltrim( $file, '/' );
      $override = apply_filters( 'fpblock_override', 'fpblock-override' );
    ...
    public static function includes() {
      ...
      $fields = apply_filters( 'fpblock_fields', array(
        'accordion',
        'background',
        'backup',
        'border',
      ...
      if ( ! empty( $fields ) ) {
        foreach ( $fields as $field ) {
          if ( ! class_exists( 'FPBLOCK_Field_'. $field ) && class_exists( 'FPBLOCK_Fields' ) ) {
    ...
    public static function textdomain() {
      load_textdomain( 'fpblock', self::$dir .'/languages/'. get_locale() .'.mo' );
    }
    ...
    public static function add_admin_enqueue_scripts() {
      ...
      if ( ! self::$enqueue ) {
        ...
        if ( $wpscreen->id === 'tools_page_fpblock-welcome' ) {
          self::$enqueue = true;
        }

      }

      if ( ! apply_filters( 'fpblock_enqueue_assets', self::$enqueue ) ) {
        return;
      }
      ...
      if ( apply_filters( 'fpblock_fa4', false ) ) {
        wp_enqueue_style( 'fpblock-fa', 'https://cdn.jsdelivr.net/npm/font-awesome@4.7.0/css/font-awesome.min.css', array(), '4.7.0', 'all' );
      } else {
      ...
      // Main style
      wp_enqueue_style( 'fpblock', self::include_plugin_url( 'assets/css/style'. $min .'.css' ), array(), self::$version, 'all' );

      // Main RTL styles
      if ( is_rtl() ) {
        wp_enqueue_style( 'fpblock-rtl', self::include_plugin_url( 'assets/css/style-rtl'. $min .'.css' ), array(), self::$version, 'all' );
      }

      // Main scripts
      wp_enqueue_script( 'fpblock-plugins', self::include_plugin_url( 'assets/js/plugins'. $min .'.js' ), array(), self::$version, true );
      wp_enqueue_script( 'fpblock', self::include_plugin_url( 'assets/js/main'. $min .'.js' ), array( 'fpblock-plugins' ), self::$version, true );

      // Main variables
      wp_localize_script( 'fpblock', 'fpblock_vars', array(
        'color_palette'     => apply_filters( 'fpblock_color_palette', array() ),
        'i18n'              => array(
          'confirm'         => esc_html__( 'Are you sure?', 'fpblock' ),
          'typing_text'     => esc_html__( 'Please enter %s or more characters', 'fpblock' ),
          'searching_text'  => esc_html__( 'Searching...', 'fpblock' ),
          'no_results_text' => esc_html__( 'No results found.', 'fpblock' ),
        ),
      ) );
      ...
      if ( ! empty( self::$fields ) ) {
        foreach ( self::$fields as $field ) {
          if ( ! empty( $field['type'] ) ) {
            $classname = 'FPBLOCK_Field_' . $field['type'];
    ...
    public static function add_typography_enqueue_styles() {
      ...
      if ( ! empty( self::$webfonts ) ) {
        ...
        if ( ! empty( self::$webfonts['enqueue'] ) ) {
          ...
          wp_enqueue_style( 'fpblock-google-web-fonts', esc_url( add_query_arg( $query, '//fonts.googleapis.com/css' ) ), array(), null );
        ...
        if ( ! empty( self::$webfonts['async'] ) ) {
          ...
          wp_enqueue_script( 'fpblock-google-web-fonts', esc_url( '//ajax.googleapis.com/ajax/libs/webfont/1.6.26/webfont.js' ), array(), null );

          wp_localize_script( 'fpblock-google-web-fonts', 'WebFontConfig', array( 'google' => array( 'families' => $fonts ) ) );
    ...
    public static function field( $field = array(), $value = '', $unique = '', $where = '', $parent = '' ) {
      ...
      if ( ! empty( $field['_notice'] ) ) {
        ...
        $field            = array();
        $field['content'] = esc_html__( 'Oops! Not allowed.', 'fpblock' ) .' <strong>('. $field_type .')</strong>';
        $field['type']    = 'notice';
        $field['style']   = 'danger';
      ...
      $is_pseudo  = ( ! empty( $field['pseudo'] ) ) ? ' fpblock-pseudo-field' : '';
      ...
      if ( ! empty( $field['dependency'] ) ) {
        ...
        $visible = ( ! empty( $depend_visible ) ) ? ' fpblock-depend-visible' : ' fpblock-depend-hidden';

      }

      // These attributes has been sanitized above.
      echo '<div class="fpblock-field fpblock-field-'. $field_type . $is_pseudo . $class . $visible .'"'. $depend .'>';

      if ( ! empty( $field_type ) ) {

        if ( ! empty( $field['title'] ) ) {
          echo '<div class="fpblock-title">';
          echo '<h4>'. $field['title'] .'</h4>';
          echo ( ! empty( $field['subtitle'] ) ) ? '<div class="fpblock-subtitle-text">'. $field['subtitle'] .'</div>' : '';
          echo '</div>';
        }

        echo ( ! empty( $field['title'] ) ) ? '<div class="fpblock-fieldset">' : '';

        $value = ( ! isset( $value ) && isset( $field['default'] ) ) ? $field['default'] : $value;
        $value = ( isset( $field['value'] ) ) ? $field['value'] : $value;

        $classname = 'FPBLOCK_Field_'. $field_type;

        if ( class_exists( $classname ) ) {
          $instance = new $classname( $field, $value, $unique, $where, $parent );
          $instance->render();
        } else {
          echo '<p>'. esc_html__( 'Field not found!', 'fpblock' ) .'</p>';
        }
...
FPBLOCK_Setup::init( __FILE__, true );
 ...
 *
 */
if ( ! class_exists( 'FPBLOCK' ) ) {
  class FPBLOCK extends FPBLOCK_Setup{}
}
